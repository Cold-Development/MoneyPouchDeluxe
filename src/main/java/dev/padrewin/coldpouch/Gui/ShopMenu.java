package dev.padrewin.coldpouch.Gui;
...
public class ShopMenu extends Menu {
    ...
    public ShopMenu(Player player, ColdPouch plugin) {
        super(player);
        this.plugin = plugin;
    }
    ...
    public void onClick(int slot) {
        ...
        if (super.getInventory().firstEmpty() == -1) {
            super.getPlayer().sendMessage(plugin.getMessage(ColdPouch.Message.INVENTORY_FULL));
        } else if (pouch != null) {
            boolean success;
            try {
                success = pouch.getPurchaseCurrency().doTransaction(super.getPlayer(), pouch.getPurchasePrice());
            } catch (Exception e) {
                super.getPlayer().sendMessage(plugin.getMessage(ColdPouch.Message.PURCHASE_ERROR)
            ...
            if (success) {
                ...
                super.getPlayer().sendMessage(plugin.getMessage(ColdPouch.Message.PURCHASE_SUCCESS)
                        .replace("%item%", pouch.getItemStack().getItemMeta().getDisplayName())
                        .replace("%prefix%", pouch.getPurchaseCurrency().getPrefix())
                        .replace("%suffix%", pouch.getPurchaseCurrency().getSuffix())
                        .replace("%price%", String.valueOf(pouch.getPurchasePrice())));
            } else {
                super.getPlayer().sendMessage(plugin.getMessage(ColdPouch.Message.PURCHASE_FAIL)
                        .replace("%prefix%", pouch.getPurchaseCurrency().getPrefix())
                        .replace("%suffix%", pouch.getPurchaseCurrency().getSuffix())
                        .replace("%price%", String.valueOf(pouch.getPurchasePrice())));
            }
